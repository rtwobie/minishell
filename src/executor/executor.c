...
static int	_exec_cmd(t_command_node *cmd, int fd_in, int fd_out)
{
	pid_t	pid;
	char	*program;
	...
	if (is_builtin(cmd->program_argv[0]))
		return (_exec_builtin(cmd, fd_in, fd_out));
...
static int	_handle_pipe(t_ast_node *node, int fdin, int fdout);

static int	_exec(t_ast_node *node, int fdin, int fdout)
{
	if (node == NULL)
		return (EXIT_FAILURE);
	if (node->type == NODE_TYPE_COMMAND)
		return (_exec_cmd(node->data.command_node, fdin, fdout));
	else if (node->type == NODE_TYPE_PIPE)
		return (_handle_pipe(node, fdin, fdout));
	return (EXIT_FAILURE);
}
...
static int	_handle_pipe(t_ast_node *node, int fdin, int fdout)
{
	...
	{
		(close(pfd[0]), st = _exec(node->data.pipe_node->left, fdin, pfd[1]));
	...
	{
		(close(pfd[1]), st = _exec(node->data.pipe_node->right, pfd[0], fdout));
		(rl_clear_history(), exit(st));
	}
...
int	executor(t_ast_node **tree, unsigned char *exit_status, char **envp)
{
	...
	*exit_status = (unsigned char)_exec(*tree, STDIN_FILENO, STDOUT_FILENO);
